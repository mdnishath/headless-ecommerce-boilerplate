...
> **Review amendment:** original snippet used loose `length(3)`/`min(2)` string checks. Probes showed `"usd"`, lowercase country keys, `{DE: "JPY"}` (currency not in `currencies`), `ftp://` endpoints, and `"EN!"` locales all passed — each a silent failure waiting for Phase 3 middleware (geo headers are uppercase; comparisons are case-sensitive). Tightened to format regexes + a third referential-integrity refine.
...
Note: `z.url({ protocol })` and two-argument `z.record(key, value)` are zod v4 syntax (v4 validates record KEYS — pinned by a regression test).
